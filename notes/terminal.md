all notes about TERMINAL COMMANDS, GREP, FIND, ETC. go here:


ipconfig
arp-a /address resolution protocol, all ip addresses
ping -t /extended ping tests latency
ipconfig
arp-a /address resolution protocol, all ip addresses
ping -t /extended ping tests latency
...
========== Terminal Commands =============
CMD K to clear scrollback text

clear shifts all text in window out of view into scrollback area
CMD K: same thing
...
k clears before cursor
a go to start
e go to end
c KILLS whatever. you are running
r search for previous commands

SHOW FILE TYPES:
ls -l | sort -k1,1
find DirABC/ -maxdepth 1 -type d -ls
find . -not -type d =finds all files except directories
ls -l | grep ^d =finds only directories
